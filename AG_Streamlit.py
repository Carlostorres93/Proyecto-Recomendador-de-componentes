...
PS = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/power-supply_clean.csv')
CASE = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/case_clean.csv')
MB = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/motherboard_clean.csv')
...
Para simplifiFerrao, se puede decir que los orígenes más recientes de la industria informática se remontan a finales de los años 1940. El hecho es que la Leland Stanford Junior University, con el objeto de conseguir dinero con el que poder atraer a buenos profesores para impulsar el desarrollo de la universidad, se les ocurrió crear el Parque Industrial Stanford alquilando los terrenos de la universidad.
...
A principios de la década de 1980 IBM elige la arquitectura de chip x86 de Intel para construir su famoso IBM PC. No obstante, como IBM no quería depender de un solo proveedor, le exige a Intel que licencie la fabricación de sus microprocesadores para que puedan fabriFerraos otras empresas. Entre ellas, estaba AMD.
...
Esto da mucha rapidez porque permite que los datos se lean o escriban de manera mucho más rápida ya que no hay que busFerraos de manera secuencial. Nos creas o no, el hacer la inserción y lectura de manera aleatoria por los programas, hacen que los conflictos cuando se intente acceder a los datos por dos aplicaciones diferentes a la misma celda se minimice exponencialmente. 
...
A continuación te ayudamos a distinguir entre los diferentes tipos de cajas de PC para que a partir de ahora puedas identifiFerraos sin ningún problema.
...
def mostrar_ml():
    ...
    # Leer el DataFrame
    data_CPU = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/cpu_ferran_clean.csv')  
    data_GPU = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/gpu_ferran_clean.csv')
    data_SSD = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/ssd_limpio.csv')
    data_RAM = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/Ram_clean.csv')
...
def algoritmo_genetico(presupuesto, CPU, GPU, SSD, RAM):
    ...
    F_Alim = 62 if GPU['Chip'][mejor_pc_posiciones[1]] == 'GeForce RTX 4090' or GPU['Chip'][mejor_pc_posiciones[1]] == 'Radeon RX 7900 XTX' else 166
    ...
    Mb_ex = 62 if Cpu_zocalo == 'AM5' else 241 if Cpu_zocalo == 'AM4' else 191 if Cpu_zocalo == 'LGA1200' else 86 if Cpu_zocalo == 'LGA1700' else None

    return mejor_pc, fitness, F_Alim, Cpu_zocalo, Mb_ex

def mostrar_configurador():
    CPU = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/cpu_ferran_clean.csv')
    GPU = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/gpu_ferran_clean.csv')
    SSD = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/SSD_limpio.csv')
    RAM = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/RAM_clean.csv')
    PS = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/power-supply_clean.csv')
    CASE = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/case_clean.csv')
    MB = pd.read_csv('C:/Users/mary_/Downloads/Pablo/Data Scientist/Archivos_Finales/motherboard_clean.csv')
    ...
    if st.button("Haga click aquí para configurar su PC"):
        ...
        cpu_nombre = mejor_pc[0]['Nombre']
        Cpu_zocalo = 'AM5' if 'AMD' in cpu_nombre and ' 7' in cpu_nombre else 'AM4' if 'AMD' in cpu_nombre and ' 7' not in cpu_nombre else 'LGA1200' if '-10' in cpu_nombre or '-11' in cpu_nombre else 'LGA1700' if '-12' in cpu_nombre or '-13' in cpu_nombre or '-14' in cpu_nombre else None
        Mb_ex = 62 if Cpu_zocalo == 'AM5' else 241 if Cpu_zocalo == 'AM4' else 191 if Cpu_zocalo == 'LGA1200' else 86 if Cpu_zocalo == 'LGA1700' else None
        F_Alim = 62 if mejor_pc[1]['Nombre'] == 'GeForce RTX 4090' or mejor_pc[1]['Nombre'] == 'Radeon RX 7900 XTX' else 166
        if mejor_pc is not None:
            ...
            st.write("Memoria RAM:", mejor_pc[3]['Nombre'], mejor_pc[3]['Velocidad'], mejor_pc[3]['Precio'], "€")
            st.write("--------------------------------------------------")
            st.write("Fuente de alimentación:", PS.loc[F_Alim, 'Nombre'], PS.loc[F_Alim, 'Precio'], "€")
            st.write("Caja:", CASE.loc[4, 'Nombre'], CASE.loc[4, 'Precio'], "€")
            st.write("Placa base:", MB.loc[Mb_ex, 'Nombre'], MB.loc[Mb_ex, 'Precio'], "€")
            precio_total_componentes = sum([mejor_pc[i]['Precio'] for i in range(4)])
            precio_total_adicional = PS.loc[F_Alim, 'Precio'] + CASE.loc[4, 'Precio'] + MB.loc[Mb_ex, 'Precio']
